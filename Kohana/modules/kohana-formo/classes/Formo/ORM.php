...
class Formo_ORM extends ORM{
	...
	public function get_form( array $fields, Formo $form = NULL)
	{
		...
		if ($form === NULL)
		{
			...
			$form = Formo::form(array('alias' => $alias));
		}
		...
		foreach ($fields as $key => $field_name)
		{
			...
			if ($this->_field_exists($field_name) !== TRUE)
			{
				...
				if ( ! method_exists($this, $method))
				{
					...
					throw new Kohana_Exception('Formo custom field method, :method, does not exist.', array(':method' => __CLASS__.'::'.$method.'()'));
				}
		...
		if ($filters = $this->filters())
		{
			...
			{
				// Add filters as well
				$form->merge($alias, array('filters' => $_filters));
			}
		}
	...
	{
		$field_alias = Arr::path($this->_foreign_keys, 'belongs_to.'.$field_name, array());
	...
	{
		$field_name = Arr::path($this->_foreign_keys, 'has_many.'.$field_name, array());
	...
	{
		$column = Arr::get($this->_table_columns, $field_name, array());
