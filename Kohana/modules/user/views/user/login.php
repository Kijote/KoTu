...
echo '<table>';
echo '<tr><td>'.$form->label('username', __('Email or Username')).'</td>';
echo '<td>'.$form->input('username', null, array('class' => 'text twothirds')).'</td></tr>';
echo '<tr><td>'.$form->label('password', __('Password')).'</td>';
echo '<td>'.$form->password('password', null, array('class' => 'text twothirds')).'</td></tr>';
echo '<tr><td colspan=2>'.$form->submit(NULL, __('Login')).'</td></tr>';
echo '<tr><td colspan=2><small> '.Html::anchor('user/forgot', __('Forgot your password?')).'<br></small></td></tr>';
echo '</table>';
echo $form->close();
...
$options = array_filter((array) Kohana::$config->load('useradmin.providers'));
if(!empty($options)) {
