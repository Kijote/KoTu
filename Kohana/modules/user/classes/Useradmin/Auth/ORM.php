...
class Useradmin_Auth_ORM extends Kohana_Auth_ORM implements Useradmin_Driver_iAuth {
	...
	protected function _login($user, $password, $remember) 
	{
		...
		if ((Kohana::$config->load("useradmin.max_failed_logins") > 0) &&
			($user->failed_login_count >= Kohana::$config->load("useradmin.max_failed_logins") ) &&
			(strtotime($user->last_failed_login) > strtotime("-".Kohana::$config->load("useradmin.login_jail_time") ) )) 
		{
